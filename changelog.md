...
- `Shift+←/→` 与 `H/L` 后退/前进 60 秒；不区分 Shift+方向键的终端可用 `H/L`
- 数字键 `0`–`9` 跳到当前曲目的 0%–90% 位置
- `[`/`]` 以 1% 为步长细调音量
- 帮助弹层把关闭提示移到标题栏，24 行终端仍能完整显示

### 同步歌词

- 读取与音频同名的 `.lrc`，没有时再读取标签内嵌的 LRC 文本（LYRICS / USLT / ©lyr）；纯文本歌词不显示
- 曲库区域宽度不少于 96 列时在右侧显示滚动歌词面板，当前行居中高亮；更窄时在播放区底边显示当前行
- 支持一行多个时间标签、`[offset:±ms]`、逐字时间标签，同一时刻的原文与译文一起高亮
- `.lrc` 按 BOM 识别 UTF-8 / UTF-16；没有 BOM 时先按 UTF-8，不合法再按 GBK（GB18030）解码；都失败则忽略并提示
- `y` 开关歌词，选择以 `lyrics_enabled` 随配置保存；重扫后重新读取歌词
- 帮助弹层把频谱与歌词开关合为一行

### 依赖

- 直接依赖 `encoding_rs` 0.8.35，用于解码 GBK 与 UTF-16 歌词；它原本已经经由 lofty / symphonia 编入程序，`Cargo.lock` 没有新增包，许可证（(Apache-2.0 OR MIT) AND BSD-3-Clause）已在第三方声明中

### 会话恢复

- 退出时把当前曲目与进度写入数据目录的 `session.toml`；下次打开同一音乐库、首次扫描完成后，暂停在该曲目的原位置，按空格续播，不会自动出声
- 距结尾不足 3 秒的曲目从头恢复；曲目已不在曲库、换了音乐库或文件打不开时安静放弃
- 首次扫描完成前就退出时保留原会话；退出时没有已载入的曲目则清除会话
- 会话文件损坏或版本不符时直接忽略，保存失败只在退出时提示，不影响退出状态
