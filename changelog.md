...
- 音频输出故障时暂停并保留歌曲、进度和队列，不再当作坏歌跳过；按空格或系统 Play 在后台重新打开默认输出设备并续播，恢复失败可再次重试
- 短暂音频缓冲欠载只提示一次，不触发跳歌；故障期间禁止切歌和 Seek，避免继续使用失效输出流

### 界面

- 播放和暂停操作图标改为 `⏵` 与文本样式的 `⏸︎`，保持三列图标区域
- 图标宽度检查改为计算显示宽度，正确处理零宽文本样式选择符

## [1.3.0] - 2026-09-07

### 修复

- 播放器改为用调用方交来的路径标识当前曲目，不再用 canonicalize 的结果：
  Windows 的 `\\?\` 前缀和 Linux 软链接的库根目录都会让重扫或重排之后
  丢失正在播放的曲目

### 鼠标

- 新增鼠标支持，默认开启：曲库和弹层列表中单击选中、双击播放，滚轮移动选择，点击进度条跳转
- `M` 开关鼠标，选择随配置保存；关闭后终端恢复自身的拖拽选区与复制
- 鼠标移动事件在事件层丢弃，指针划过终端不会持续唤醒主循环
- 播放列表与列表内容面板改用与队列面板一致的视口，底部提示行不再盖住条目

### 曲库

- 曲库可按路径、标题、歌手、专辑或时长排序，`o` 切换字段、`O` 切换升降序，选择随配置保存
- 顺序播放跟随列表当前顺序；重排后正在播放的曲目保持不变，随机袋按新下标重建
- 缺失的歌手、专辑、时长永远排在最后，不随升降序翻转
- 曲目新增音轨号和碟号，按专辑排序时专辑内按碟号、音轨号排列
- 索引缓存升到 v2：从 v1 升级时就地重建，不提示损坏也不留备份文件

### 界面

- 新增播放队列面板：`Q` 打开，可查看队列内容、`Enter` 跳到某一条播放、`d` 移除单项、`J/K` 上下移动、`c` 清空
- 队列条目显示曲名、歌手和时长；已不在曲库中的条目标记为“不在曲库中”，与播放时自动跳过的行为一致
- 帮助弹层和底部提示补上 `Q` 与 `o`

### 修复

- 自动跳过无法播放的歌曲后保留提示：切歌成功不再把跳过原因一起清掉
- 跳过多首时汇总成一条计数提示，播放中途解码出错的原因也并入同一条
- 播放失败提示去掉绝对路径，改为点名曲目加原因，避免路径把原因挤出屏幕
- `p` 回退历史时跳过失效条目不再完全静默
...
- 退出时先释放 SMTC 再卸载 WinRT，避免 COM 对象在反初始化之后仍然存活
