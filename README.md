...
- LRC 同步歌词：读取同名 `.lrc` 或内嵌歌词，宽终端在曲库右侧滚动显示，窄终端在播放区边框显示当前行
...
歌词优先读取与音频同名的 `.lrc`（如 `歌曲.flac` 旁的 `歌曲.lrc`），其次读取音频标签中内嵌的 LRC 文本；没有时间标签的纯文本歌词不显示。`.lrc` 可为 UTF-8、GBK，或带 BOM 的 UTF-16；Big5、Shift_JIS 等其他编码不受支持，可能显示为乱码。支持一行多个时间标签、`[offset:±毫秒]` 和逐字时间标签；同一时刻的多行（如原文与译文）一起高亮。新放入的 `.lrc` 在按 `r` 重扫后生效。`y` 的开关状态随配置保存为 `lyrics_enabled`。
...
sudo dnf install ./packaging/rpmbuild/RPMS/x86_64/music-player-1.3.1-1.fc44.x86_64.rpm
```
