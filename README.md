...
歌词优先读取与音频同名的 `.lrc`（如 `歌曲.flac` 旁的 `歌曲.lrc`），其次读取音频标签中内嵌的 LRC 文本；没有时间标签的纯文本歌词不显示。`.lrc` 需为 UTF-8（可带 BOM）或带 BOM 的 UTF-16，GBK 等其他编码会被忽略并提示。支持一行多个时间标签、`[offset:±毫秒]` 和逐字时间标签；同一时刻的多行（如原文与译文）一起高亮。新放入的 `.lrc` 在按 `r` 重扫后生效。`y` 的开关状态随配置保存为 `lyrics_enabled`。
